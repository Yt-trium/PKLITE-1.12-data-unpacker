	...
	const int *get(const std::vector<bool> &bits)
	{
		...
		for (const bool bit : bits)
		{
			...
			if (bit)
			{
				// Right.
				if(right == nullptr)
					std::cout << "Bit Tree : No right branch." << std::endl;
			...
			else
			{
				// Left.
				if(left == nullptr)
					std::cout << "Bit Tree : No left branch." << std::endl;
					// Check if it's a leaf.
...
int pklite_du(std::string filename)
{
	std::cout << "Exe Unpacker : Unpacking \"" << filename << "\"." << std::endl;
    ...
    {
        stream->open((*piter+(filename.c_str())).c_str(), std::ios_base::binary);
        if(stream->good()) return IStreamPtr(std::move(stream));
        ++piter;
    }

	gIStreamPtr stream = GlobalBsa.open(name);
	*/

	if(stream == nullptr)
		std::cout << "Exe Unpacker : Could not open \"" << filename << "\"." << std::endl;
	...
	for (unsigned int i = 12; i < Duplication1.size(); ++i)
	{
		bitTree1.insert(Duplication1.at(i), i + 1);
	}

	for (unsigned int i = 0; i < Duplication2.size(); ++i)
	{
	...
	const uint16_t lastCompWord = getLE16(compressedEnd - 2);
	
	if(lastCompWord != 0xFFFF)
		std::cout << "Exe Unpacker : Invalid last compressed word \"" << std::hex << (lastCompWord) << "\"." << std::endl;
	
	// Calculate length of decompressed data -- more precise method (for A.EXE).
	const size_t decompLen = [compressedEnd]()
	{
		const uint16_t segment = getLE16(compressedEnd);
		const uint16_t offset = getLE16(compressedEnd + 2);
	...
	uint16_t bitArray = getLE16(compressedStart);
	...
	*/
	std::string text;
	text.resize(decomp.size());
	std::copy(decomp.begin(), decomp.end(), text.begin());
	
	std::ofstream *stream_out = new std::ofstream();
	stream_out->open("data.txt",std::ofstream::binary);
	stream_out->write(text.c_str(),text.length());
	
	if(stream_out == nullptr)
		std::cout << "Exe Unpacker : Could not open \"" << "data.txt" << "\"." << std::endl;
		
	std::cout << "writing output file data.txt ..." << std::endl;
